...
export default function VendorList({ auth }) {
    ...
    const handleGenerateInvoice = (id) => {
        window.open(`/quotation/generate-invoice-hotel/${id}`, '_blank');
    };
    ...
    const confirmStatusChange = async () => {
        ...
        try {
            ...
            axios.post('/qoutation/update/status', {
                id: itemId,
                status: value
            });
    ...
    const copyClientDetailsToClipboard = () => {
        if (selectedClient) {
            const clientDetailsText = `
                ...
                Hotel Name: ${selectedClient.guest_name}
                Room Category: ${selectedClient.room_category}
                Hotel Address: ${selectedClient.hotel_address}
                Check In: ${selectedClient.check_in}
                Checkout: ${selectedClient.check_out}
                ...
                <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md mx-auto">
                    ...
                    {selectedClient && (
                        <div className="bg-gray-100 p-4 rounded-lg shadow-md">
                            ...
                            <p className="text-gray-600">Hotel Name: {selectedClient.guest_name}</p>
                            <p className="text-gray-600">Room Category: {selectedClient.room_category}</p>
                            <p className="text-gray-600">Hotel Address: {selectedClient.hotel_address}</p>
                            <p className="text-gray-600">Checkin Date: {selectedClient.check_in}</p>
                            <p className="text-gray-600">Checkout Date: {selectedClient.check_out}</p>
